  ...
  try {
    ...
    notify('Wallet connection cancelled');
        ...
        {active === 'My Tokens' && <PageShell title="Token portfolio" subtitle="Everything created or watched in your MoonPad workspace."><div className="toolbar"><div className="stat"><small>Total tracked</small><b>3</b></div><div className="stat"><small>24h volume</small><b>$19.7K</b></div><div className="stat"><small>Portfolio P&amp;L</small><b className="up">+$2,184</b></div></div><div className="token-list">{filteredTokens.map(t => <TokenCard key={t.ticker} t={t} large/>)}</div></PageShell>}
         ...
         {active === 'AI Agents' && <PageShell title="AI agent workspace" subtitle="Monitor markets and prepare trades for Phantom approval."><div className="agent-grid"><AgentCard icon={<Bot size={20}/>} name="Agent #001" type="Market Monitor" running={running} onToggle={() => setRunning(!running)} description="Price, volume, liquidity and holder changes"/><AgentCard icon={<Sparkles size={20}/>} name="Agent #002" type="Community Monitor" running={true} description="Community activity and mention tracking"/></div>

<div className="bot-panel">
  <div>
    <span className="eyebrow">MANUAL TRADING</span>
    <h3>Trade with Phantom</h3>
    <p>Enter a token mint and amount. MoonPad will prepare the trade for your Phantom approval.</p>
  </div>
  ...
  <div className="trade-controls">
    ...
    >
      ...
      {botEnabled ? 'Enabled' : 'Enable'}
        ...
        {active === 'Settings' && <PageShell title="Settings" subtitle="Workspace preferences and connection controls."><div className="settings-grid"><div className="setting"><Gauge size={18}/><div><b>Network</b><span>Solana · Mainnet</span></div></div><div className="setting"><Wallet size={18}/><div><b>Wallet</b><span>{connected ? `${walletAddress.slice(0, 4)}...${walletAddress.slice(-4)} connected` : 'No wallet connected'}</span></div></div><div className="setting"><Zap size={18}/><div><b>Execution</b><span>Real transactions require Phantom approval</span></div></div></div></PageShell>}
